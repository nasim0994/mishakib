...
export default function EditWhyChoose() {
  ...
  const { data } = useGetWhyChooseByIdQuery(id);
  const whyChoose: IWhyChoose = data?.data;

  useEffect(() => {
    form.setValue("title", whyChoose?.title);
  }, [whyChoose, form]);

  const [updateWhyChoose, { isLoading }] = useUpdateWhyChooseMutation();
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    const res = (await updateWhyChoose({ id, formData })) as TResponse;

    if (res?.data?.success) {
      toast.success("Why-Choose update Success");
